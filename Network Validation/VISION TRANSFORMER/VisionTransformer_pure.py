...
# EXPLICAR A MUDANÇA DA VERSÃO PURA (SEM O RESNET)
class VisionTransformer(nn.Module):
    ...
    def __call__(self, inputs, *, train):
        """Builds ViT forward pass.

        Args:
          inputs: image tensor shape (batch, H, W, C)
          train: whether in training mode (controls dropout)
        Returns:
          logits (batch, num_classes) if num_classes provided, else pre-logits.
        """
        x = inputs  # expect shape (n, H, W, C)

        # Pure ViT: patch embedding with a conv (kernel=patch_size, stride=patch_size)
        x = nn.Conv(
        ...
        # Reshape to sequence of patch embeddings: (n, h*w, c)
