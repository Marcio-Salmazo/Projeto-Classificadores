...
def train_vit(
    ...
    def log(msg: str):
        ...
        - Para a UI (PyQt) se log_callback existir
        - Para o terminal (print) caso contrário
        """
    ...
    if mode == "finetune":
        ...
        log(f">> Carregando pesos pré-treinados de {pretrained_path}")
        params = load_vit_npz(params, pretrained_path)
        log(">> Pesos pré-treinados carregados com sucesso!")
    ...
    for step in range(total_steps):
        ...
        if step % 5000 == 0 and step > 0:
            ...
            if log_callback:
                log_callback("\nVALIDAÇÃO")
                log_callback(
                    f"VAL >> loss={val_log['val_loss']:.4f}, "
